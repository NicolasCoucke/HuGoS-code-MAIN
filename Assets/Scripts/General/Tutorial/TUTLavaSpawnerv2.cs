...
public class TUTLavaSpawnerv2 : MonoBehaviourPunCallbacks, IPunObservable
{
   ...
   public void SpawnSecondLavas()
    ...
    void Spawn_next_lava(Vector3 position, float start_size)
    {
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            SpawnedLava.GetComponent<TUTLavaGenerator>().Submit_radia(startradius,endradius);
    ...
    private void Calculate_radia()
    {
        ...
        if(numPlays == 6)
        {
            startradius = (3.5f * numPlays) / (2f * 3.14f);
        }
        else 
        {
