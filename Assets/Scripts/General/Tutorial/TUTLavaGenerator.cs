...
public class TUTLavaGenerator : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    float Blockpercentage = 0.8f;
    float Proportional_increment;

    private void Start()
    {
        if (PhotonNetwork.IsMasterClient)
        {
            hole = TUTLavaSpawnerv2.Register_Hole(gameObject);
            GlobalPosition = TUTLavaSpawnerv2.Get_Position(hole);
        }
    ...
    private void Update()
    {
        ...
        if (Blocked_Vertices.Count > Blockpercentage * vertices.Length)
        {
            photonView.RPC("StartSolidify", RpcTarget.All);
            photonView.RPC("IsContained", RpcTarget.All);
        }
    ...
    void IsContained()
    {
        TutorialTexts.HoleClosed = true;
        TUTLavaSpawnerv2.HoleClosed = true;
    }
    ...
    void MeshChange(Vector3[] vertices, int[] triangles, bool create, float radius)
    {
        ...
        if (create == true)
            this.GetComponentInChildren<CapsuleCollider>().radius = 0.9f*radius;
    }
