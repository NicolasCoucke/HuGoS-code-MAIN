...
public class BLOCKTUTLavaGenerator : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    private float increment = 0.0005f;
    ...
    private void Start()
    {
        ...
        {
            hole = BlockTUTLavaSpawner.Register_Hole(gameObject);
            GlobalPosition = BlockTUTLavaSpawner.Get_Position(hole);
    ...
    private void Update()
    {
        PlacedBlockPosition = Vector3.zero;
        ...
        for (int vertex = 1; vertex < vertices.Length; vertex++)
        {
            ...
            if (Physics.Raycast(vertices[0] + GlobalPosition, vertices[vertex], out hit, Vector3.Distance(vertices[0], vertices[vertex])))
            {
                ...
                {
                    int blocker = hit.transform.gameObject.GetComponent<BlockPickUp>().BlockID;

                    if (!Blockers.Contains(blocker))
                    {
                        PlacedBlockPosition = hit.transform.position;
    ...
    public void MeshGrowing()
    {
        ...
        BlockerString = BlockerString + PlacedBlockPosition.ToString();
    ...
    public void UpdateMesh(bool create)
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            //pass through what they managed to stop
            BlockTUTLavaSpawner.Update_Score(Track_null_Surface - Track_Surface, Track_Surface, MaxScore, BlockerString, hole);
